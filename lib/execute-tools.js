...
function scopedFindTasks(supabase, ctx) {
  let q = supabase.from("t_tasks").select("id, title, line_user_id, org_unit_id");
...
async function executeTool(supabase, name, args, ctx) {
  if (name === "add_task") {
    ...
    const { error } = await supabase.from("t_tasks").insert(row);
  ...
  if (name === "complete_task") {
    ...
    const { error } = await supabase.from("t_tasks").update(updateData).eq("id", tasks[0].id);
    if (error) throw error;
  ...
  if (name === "delete_task") {
    ...
    const { error } = await supabase.from("t_tasks").delete().eq("id", tasks[0].id);
    if (error) throw error;
  ...
  if (name === "add_insight") {
    ...
    const { error } = await supabase.from("t_insights").insert(row);
    if (error) throw error;
